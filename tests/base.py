...
class IsOrderedSubsetOfMismatch(object):
    ...
    def describe(self):
        return "set %r is not a subset of %r" % (self.subset, self.set)
...
class IsOrderedSubsetOf(object):
    ...
    def match(self, actual):
        if all(item in self.iterable for item in actual):
            return None
        else:
            return IsOrderedSubsetOfMismatch(actual, self.iterator)
