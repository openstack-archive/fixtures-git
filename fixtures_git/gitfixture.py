...
class GitTree(object):
    ...
    def _build_git_tree(self, graph_def, branches=[]):
        ...
        As git supports a special merge commit that ignores all previous
        history from the other tree being merged in using the 'ours' strategy.
        This is used when needing to preserve history but not interested in
        the changes from the other branch. One tool that makes extensive usage
        of this is _git-upstream. You specify this by defining
